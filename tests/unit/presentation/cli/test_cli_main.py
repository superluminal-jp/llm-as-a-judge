...
from src.llm_judge.application.services.llm_judge_service import EvaluationResult, CandidateResponse
...
class TestCLIParser:
    ...
    def test_parser_global_options(self):
        ...
        args = parser.parse_args(['--provider', 'openai', 'evaluate', 'prompt', 'response'])
        assert args.provider == 'openai'
        
        # Test judge model option
        args = parser.parse_args(['--judge-model', 'gpt-4', 'evaluate', 'prompt', 'response'])
    ...
    def test_evaluate_command_parsing(self):
        ...
        # Test with criteria
        args = parser.parse_args(['evaluate', 'prompt', 'response', '--criteria', 'accuracy'])
    ...
    def test_compare_command_parsing(self):
        ...
        args = parser.parse_args([
            'compare', 'prompt', 'resp_a', 'resp_b', 
            '--model-a', 'gpt-4', '--model-b', 'claude-3'
        ])
...
class TestEvaluateCommand:
    ...
    async def test_evaluate_command_success(self):
        ...
        with patch('src.llm_judge.presentation.cli.main.load_cli_config') as mock_config, \
             patch('src.llm_judge.presentation.cli.main.LLMJudge') as mock_judge_class:
