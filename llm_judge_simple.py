...
from fallback_manager import get_fallback_manager, FallbackResponse
...
class LLMJudge:
            ...
            try:
                ...
                if isinstance(fallback_response.content, dict):
                    ...
                    return EvaluationResult(
                        ...
                        confidence=float(result.get("confidence", fallback_response.confidence))
                    )
                else:
                ...
                logging.error(f"{self.config.default_provider.title()} evaluation failed: {e}")
                # Fall back to mock
    ...
    async def compare_responses(self, response_a: CandidateResponse, response_b: CandidateResponse) -> Dict[str, Any]:
        ...
        if self._use_real_llm:
            ...
            try:
                ...
                logging.error(f"{self.config.default_provider.title()} comparison failed: {e}")
                # Fall back to mock
    ...
    def _create_evaluation_prompt(self, candidate: CandidateResponse, criteria: str) -> str:
...
Be thorough and fair."""
    ...
    def _create_comparison_prompt(self, response_a: CandidateResponse, response_b: CandidateResponse) -> str:
