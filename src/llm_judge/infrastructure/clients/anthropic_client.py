...
class AnthropicClient:
    ...
    def __init__(self, config: LLMConfig):
        ...
        self.logger.info(f"Initialized Anthropic client for model: {config.anthropic_model}")
    ...
    async def create_message(
        ...
        **kwargs
        ...
        if temperature is not None and top_p is not None:
            self.logger.warning("Both temperature and top_p specified in parameters. Using temperature only.")
            ...
            self.logger.warning("Both temperature and top_p configured. Using temperature only.")
        ...
        async def _make_message():
            ...
            self.logger.debug(f"Anthropic message request: model={model}, messages={len(anthropic_messages)}")
            ...
            self.logger.debug(f"Anthropic response: {len(content)} chars, stop_reason={stop_reason}")
        ...
        try:
            ...
            if hasattr(self.logger, 'log_api_call'):
                self.logger.log_api_call("anthropic", "create_message", 0, True,
                                       model=model,
            ...
            if hasattr(self.logger, 'log_api_call'):
                self.logger.log_api_call("anthropic", "create_message", 0, False,
...
Be thorough and fair in your evaluation."""
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Anthropic evaluation: score={score}, confidence={confidence}")
                return evaluation
                
            except (json.JSONDecodeError, ValueError) as e:
                self.logger.warning(f"Failed to parse Anthropic evaluation response: {e}")
...
Be thorough in your comparison."""
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Anthropic comparison: winner={winner}, confidence={confidence}")
                return comparison
                
            except (json.JSONDecodeError, ValueError) as e:
                self.logger.warning(f"Failed to parse Anthropic comparison response: {e}")
                self.logger.warning(f"Raw response: {response.content}")
