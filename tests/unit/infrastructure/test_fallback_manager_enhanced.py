...
class TestFallbackManagerInitialization:
    ...
    async def test_initialize_providers(self, fallback_manager):
        ...
        for (
            provider_name,
            health,
        ) in fallback_manager.health_monitor.provider_health.items():
...
class TestFallbackManagerExecution:
    ...
    async def test_execute_with_fallback_success_first_provider(self, fallback_manager):
        ...
        mock_operation = AsyncMock(
            return_value={"result": "success", "provider": "anthropic"}
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_with_preferred_provider_not_available(
        ...
        result = await fallback_manager.execute_with_fallback(
            mock_operation, context, preferred_provider="nonexistent"
        )
...
class TestFallbackManagerIntegration:
    ...
    async def test_health_monitoring_affects_provider_selection(self, fallback_manager):
        ...
        for _ in range(10):
            fallback_manager.health_monitor.record_failure(
                "anthropic", Exception("Test error")
            )
    ...
    async def test_caching_prevents_duplicate_requests(self, fallback_manager):
        ...
        # Mock operation
        mock_operation = AsyncMock(
            return_value={"score": 4.5, "reasoning": "Cached response"}
        )
    ...
    async def test_service_mode_determination(self, fallback_manager):
        ...
        for provider in fallback_manager.provider_priority:
            for _ in range(10):
                fallback_manager.health_monitor.record_failure(
                    provider, Exception("All failing")
                )
...
class TestFallbackManagerErrorScenarios:
    ...
    async def test_concurrent_requests(self, fallback_manager):
        ...
        # Create multiple concurrent requests with unique contexts
        tasks = []
        for i in range(5):
            unique_context = {
                "type": "evaluation",
                "prompt": f"test request {i}",  # Make each request unique
                "request_id": i,
            }
            task = fallback_manager.execute_with_fallback(
                mock_operation, unique_context, preferred_provider="anthropic"
            )
            tasks.append(task)
    ...
    @pytest.mark.asyncio
    async def test_provider_priority_with_no_preferred(self, fallback_manager):
    ...
    @pytest.mark.asyncio
    async def test_provider_priority_with_invalid_preferred(self, fallback_manager):
