...
class HealthMonitor:
    ...
    def record_success(self, provider_name: str, response_time: float) -> None:
        ...
        if health.avg_response_time == 0:
            ...
            health.avg_response_time = (
                health.avg_response_time * 0.8 + response_time * 0.2
            )
    ...
    def _update_provider_status(self, provider_name: str) -> None:
        ...
        if old_status != health.status:
            self.logger.warning(
                f"Provider {provider_name} status changed: {old_status.value} -> {health.status.value}"
            )
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        if not self.provider_health:
            return {"status": "unknown", "providers": {}, "overall_success_rate": 0.0}
        ...
        total_requests = sum(
            health.total_requests for health in self.provider_health.values()
        )
        total_failures = sum(
            health.failed_requests for health in self.provider_health.values()
        )
        ...
        while True:
            try:
                ...
                for provider_name, health in self.provider_health.items():
                    ...
                    last_activity = max(
                        health.last_success or 0, health.last_failure or 0
                    )
                    ...
                    if current_time - last_activity > 600:  # 10 minutes
                        if health.status != ProviderStatus.MAINTENANCE:
                            ...
                            if old_status != health.status:
                                self.logger.warning(
                                    f"Provider {provider_name} marked as unavailable due to inactivity"
                                )

                # Log system health summary
                system_health = self.get_system_health()
                self.logger.info(
                    f"System health check: {system_health['status']} "
...
class FallbackManager:
    ...
    async def execute_with_fallback(
        ...
        for provider in providers_to_try:
            try:
                ...
                return FallbackResponse(
                    ...
                    confidence=1.0,
                    success=True,
                    attempts_made=providers_to_try.index(provider) + 1,
                    metadata={
        ...
        if self.enable_caching:
            ...
            if cached_response is not None:
                ...
                return FallbackResponse(
                    ...
                    is_simplified=False,
                    confidence=0.7,
                    success=False,  # All providers failed, so this is a fallback
                    attempts_made=len(providers_to_try),
                    metadata={"fallback": True, "fallback_reason": "cached_response"},
                )
        ...
        if self.enable_simplified_responses:
            ...
            if simplified_response is not None:
                ...
                return FallbackResponse(
                    ...
                    metadata={
                        "fallback": True,
                        "fallback_reason": "simplified_response",
                    },
                )
        ...
        return FallbackResponse(
            ...
            success=False,
            attempts_made=len(providers_to_try),
            metadata={"fallback": True, "fallback_reason": "error_response"},
        )
        ...
        if preferred_provider and preferred_provider in available_providers:
            ...
            provider_order.extend(
                [
                    ...
                    if p != preferred_provider and p in available_providers
                ]
            )
        else:
            # Use default priority order
            provider_order = [
                p for p in self.provider_priority if p in available_providers
            ]
    ...
    def _generate_simplified_response(
    ...
    async def set_maintenance_mode(
        ...
        if enabled:
            ...
            self.logger.warning(
                f"Maintenance mode enabled: {reason or 'No reason specified'}"
            )
        else:
