...
class LLMConfig:
    ...
    def _validate_config(self):
        ...
        if self.default_provider not in ["openai", "anthropic"]:
            errors.append(f"Invalid default_provider '{self.default_provider}'. Must be 'openai' or 'anthropic'.")
        ...
        if self.openai_request_timeout <= 0:
            errors.append(f"openai_request_timeout must be positive, got {self.openai_request_timeout}")
        
        if self.anthropic_request_timeout <= 0:
            errors.append(f"anthropic_request_timeout must be positive, got {self.anthropic_request_timeout}")
        
        if self.openai_connect_timeout <= 0:
            errors.append(f"openai_connect_timeout must be positive, got {self.openai_connect_timeout}")
        
        if self.anthropic_connect_timeout <= 0:
            errors.append(f"anthropic_connect_timeout must be positive, got {self.anthropic_connect_timeout}")
        ...
        if self.gpt5_reasoning_effort not in valid_reasoning_efforts:
            errors.append(f"Invalid gpt5_reasoning_effort '{self.gpt5_reasoning_effort}'. Must be one of: {valid_reasoning_efforts}")
        
        # Validate Anthropic temperature and top_p
        if self.anthropic_temperature is not None and (self.anthropic_temperature < 0 or self.anthropic_temperature > 1):
            errors.append(f"anthropic_temperature must be between 0 and 1, got {self.anthropic_temperature}")
        
        if self.anthropic_top_p is not None and (self.anthropic_top_p < 0 or self.anthropic_top_p > 1):
            errors.append(f"anthropic_top_p must be between 0 and 1, got {self.anthropic_top_p}")
        ...
        if self.anthropic_temperature is not None and self.anthropic_top_p is not None:
            errors.append("Both anthropic_temperature and anthropic_top_p are specified. Use only one (they are mutually exclusive).")
        
        # Validate retry configuration
        if self.retry_max_attempts <= 0:
            errors.append(f"retry_max_attempts must be positive, got {self.retry_max_attempts}")
        
        if self.retry_base_delay < 0:
            errors.append(f"retry_base_delay must be non-negative, got {self.retry_base_delay}")
        
        if self.retry_max_delay < self.retry_base_delay:
            errors.append(f"retry_max_delay ({self.retry_max_delay}) must be >= retry_base_delay ({self.retry_base_delay})")
        ...
        if self.circuit_breaker_failure_threshold <= 0:
            errors.append(f"circuit_breaker_failure_threshold must be positive, got {self.circuit_breaker_failure_threshold}")
        
        if self.circuit_breaker_recovery_timeout <= 0:
            errors.append(f"circuit_breaker_recovery_timeout must be positive, got {self.circuit_breaker_recovery_timeout}")
        
        if self.circuit_breaker_success_threshold <= 0:
            errors.append(f"circuit_breaker_success_threshold must be positive, got {self.circuit_breaker_success_threshold}")
...
def load_config() -> LLMConfig:
    ...
    try:
        ...
        config = LLMConfig(
            ...
            # Provider-specific timeout configuration
            openai_request_timeout=int(os.getenv("OPENAI_REQUEST_TIMEOUT", os.getenv("REQUEST_TIMEOUT", "30"))),
            anthropic_request_timeout=int(os.getenv("ANTHROPIC_REQUEST_TIMEOUT", os.getenv("REQUEST_TIMEOUT", "30"))),
    ...
    if hasattr(record, 'msg') and isinstance(record.msg, str):
        ...
        for key_name in ['api_key', 'token', 'key', 'secret']:
            if key_name in msg.lower():
                record.msg = msg.replace(record.msg, '[REDACTED - API key detected in log message]')
                break
    return True