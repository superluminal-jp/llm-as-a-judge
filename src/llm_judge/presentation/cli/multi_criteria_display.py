...
class MultiCriteriaDisplayFormatter:
    ...
    def format_evaluation_result(
        self, result: MultiCriteriaResult, output_format: str = "text"
    ) -> str:
    ...
    def _format_json_output(self, result: MultiCriteriaResult) -> str:
        ...
        output_data = {
            ...
            "overall_score": (
                result.aggregated.overall_score if result.aggregated else None
            ),
            "overall_confidence": (
            ...
            output_data.update(
                {
                    ...
                    "score_range": [
                        result.aggregated.min_score,
                        result.aggregated.max_score,
                    ],
                }
            )
    ...
    def _format_rich_output(self, result: MultiCriteriaResult) -> str:
        ...
        # Create a StringIO buffer to capture output without printing to stdout
        string_buffer = io.StringIO()
        console = Console(file=string_buffer, record=True)

        # Main header
        console.print(
            "\n[bold blue]🎯 Multi-Criteria LLM Evaluation Results[/bold blue]",
            justify="center",
        )
        ...
        if result.criterion_scores:
            ...
            for cs in sorted(
                result.criterion_scores, key=lambda x: x.score, reverse=True
            ):
                ...
                table.add_row(
                    ...
                    f"[{score_color}]{cs.score}/5[/{score_color}]",
                    f"{cs.weight:.1%}",
                    f"{cs.confidence:.1%}",
        ...
        if result.aggregated and len(result.criterion_scores) > 1:
            ...
            stats_table.add_row(
                "Score Range",
                f"{result.aggregated.min_score} - {result.aggregated.max_score}",
            )
            stats_table.add_row(
                "Standard Deviation", f"{result.aggregated.score_std:.2f}"
            )
        ...
        console.print(
            f"[dim]Evaluation Time: {result.evaluation_timestamp.strftime('%Y-%m-%d %H:%M:%S')}[/dim]"
        )
    ...
    def _format_basic_output(self, result: MultiCriteriaResult) -> str:
        ...
        if result.criterion_scores:
            ...
            for cs in sorted(
                ...
                lines.append(
                    f"\n{cs.criterion_name.upper()}: {cs.score}/5 (Weight: {cs.weight:.1%})"
                )
        ...
        if result.aggregated and len(result.criterion_scores) > 1:
            ...
            lines.append(
                f"  Range: {result.aggregated.min_score} - {result.aggregated.max_score}"
            )
        ...
        lines.append(
            f"Evaluation Time: {result.evaluation_timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    def _create_score_bar(self, score: int, max_score: int) -> str:
    ...
    def format_comparison_result(
        ...
        lines.append("")
        lines.append(
            f"Response A ({result.get('model_a', 'unknown')}): {result.get('response_a', '')}"
        )
        lines.append(
            f"Response B ({result.get('model_b', 'unknown')}): {result.get('response_b', '')}"
        )
        lines.append("")
