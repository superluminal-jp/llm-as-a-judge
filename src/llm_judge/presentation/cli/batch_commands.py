...
class BatchProgressTracker:
    ...
    def update(self, progress_data: BatchProgress):
        ...
        if self.use_rich and self.progress and self.task_id:
            ...
            percentage = (completed / progress_data.total_items) * 100 if progress_data.total_items > 0 else 0
            elapsed = time.time() - self.start_time if self.start_time else 0
            
            print(f"\rProgress: {completed}/{progress_data.total_items} ({percentage:.1f}%) "
...
class BatchCLICommands:
    ...
    def add_batch_subparser(self, subparsers):
        ...
        batch_parser.add_argument(
            'input_file',
            type=str,
            help='Input file path (JSONL, CSV, or JSON format)'
        )
        ...
        batch_parser.add_argument(
            '--no-retry',
            action='store_true',
            help='Disable retries for failed items'
        )
        ...
        batch_parser.add_argument(
            '--batch-name',
            type=str,
            help='Optional name for the batch'
        )
        ...
        sample_parser.add_argument(
            'output_file',
            type=str,
            help='Output file path for sample batch'
        )
    ...
    async def handle_create_sample_command(self, args) -> int:
        ...
        try:
            ...
            print(f"You can now edit this file and process it with: llm-judge batch {args.output_file}")
    ...
    async def _display_batch_results(self, result, args):
        ...
        if args.quiet:
            # Minimal output
            print(f"{result.completed_items_count}/{result.total_items} successful ({result.success_rate:.1%})")
            return
...
"""
        ...
        console.print(Panel(summary_text.strip(), title="[bold]Batch Results Summary[/bold]"))
        ...
        if result.failed_items_count > 0:
            ...
            for item in result.failed_items[:10]:  # Show first 10 failures
                table.add_row(
                    ...
                    item.error[:50] + "..." if item.error and len(item.error) > 50 else item.error or "Unknown"
                )
