...
CDK 依存のインストール、`cdk bootstrap`（ベストエフォート）、`LlmJudgeStack-<env>` への `cdk deploy` を順に実行する。`--env dev|prd` でスタックを切り替えられる。
...
Python CDK v2 で 1 つの Lambda 関数と IAM を定義する。エントリは [`cdk/app.py`](../cdk/app.py)（`python3 cdk/app.py`）。CFN スタック名は **`LlmJudgeStack-<environment>`**（例: `LlmJudgeStack-dev`、`LlmJudgeStack-prd`）で、`environment` ごとに別スタックとして同一アカウントに同居できる。
...
### 設定の優先順位（`environment` / `default_provider` / `criteria_bucket_arn`）
...
3. 最終フォールバック: `environment` → **`dev`**、`default_provider` → **`bedrock`**、`criteria_bucket_arn` 空なら S3 用 IAM を付けない。
...
cdk deploy LlmJudgeStack-dev --app "python3 cdk/app.py" --context environment=dev --require-approval never
```

リージョンは `AWS_REGION` または `config/parameters.json` の `aws_region`（`scripts/deploy.sh` 利用時）。`environment` は `scripts/deploy.sh --env prd` でも上書き可能。

### スタックの出力

- `LambdaFunctionArn` — 関数 ARN（エクスポート名 `LlmJudgeFunctionArn-<environment>`）
- `LambdaFunctionName` — 関数名

### スタックタグ

- `Environment` — `dev` / `prd` 等
