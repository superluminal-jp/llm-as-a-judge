...
def _normalize_optional_descriptor(raw: object, field: str) -> str | None:
    ...
    if not s:
        return None
    if len(s) > _DESCRIPTOR_MAX_LEN:
        raise ValidationError(
            f"Event field '{field}' exceeds maximum length {_DESCRIPTOR_MAX_LEN}."
        )
    for char in s:
        code = ord(char)
...
def _validate_event(
    ...
    """Validate the Lambda event and return normalised role text and descriptors.
