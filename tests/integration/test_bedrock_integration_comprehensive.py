...
def mock_bedrock_success_response():
    ...
    return {
        'output': {
            'message': {
                'content': [{'text': '{"score": 4.2, "reasoning": "Well-structured and accurate response.", "confidence": 0.85}'}]
            }
        },
        'stopReason': 'end_turn',
...
class TestBedrockIntegrationBasic:
    ...
    @pytest.mark.asyncio
    async def test_single_criterion_evaluation_integration(self, bedrock_config, sample_candidate):
    ...
    @pytest.mark.asyncio
    async def test_multi_criteria_evaluation_integration(self, bedrock_config, sample_candidate):
        ...
        with patch('src.llm_judge.infrastructure.clients.bedrock_client.BOTO3_AVAILABLE', True):
            with patch('boto3.Session') as mock_session:
                ...
                multi_criteria_response = {
                    "evaluations": [
                        {"criterion": "accuracy", "score": 4, "reasoning": "Factually correct", "confidence": 0.9},
                        {"criterion": "clarity", "score": 4, "reasoning": "Well-structured", "confidence": 0.85},
                        {"criterion": "completeness", "score": 3, "reasoning": "Could be more detailed", "confidence": 0.8}
                    ],
                    "overall_score": 3.7,
    ...
    async def test_bedrock_api_error_fallback(self, bedrock_config, sample_candidate):
        ...
        with patch('src.llm_judge.infrastructure.clients.bedrock_client.BOTO3_AVAILABLE', True):
            with patch('boto3.Session') as mock_session:
                ...
                assert "Mock evaluation" in result.reasoning or "temporarily unavailable" in result.reasoning.lower()
                
                await judge.close()
    
    @pytest.mark.asyncio
    async def test_invalid_bedrock_response_handling(self, bedrock_config, sample_candidate):
        ...
        with patch('src.llm_judge.infrastructure.clients.bedrock_client.BOTO3_AVAILABLE', True):
            with patch('boto3.Session') as mock_session:
                ...
                mock_response_body = {
                    'output': {
                        'message': {
                            'content': [{'text': 'This is not valid JSON for evaluation'}]
                        }
                    },
                    'stopReason': 'end_turn',
                    'usage': {'inputTokens': 50, 'outputTokens': 30}
    ...
    async def test_bedrock_timeout_handling(self, bedrock_config, sample_candidate):
        ...
        with patch('src.llm_judge.infrastructure.clients.bedrock_client.BOTO3_AVAILABLE', True):
            with patch('boto3.Session') as mock_session:
                ...
                # Mock timeout error
                import asyncio
                mock_client.invoke_model.side_effect = asyncio.TimeoutError("Request timed out")
...
class TestBedrockIntegrationConcurrency:
    ...
    @pytest.mark.asyncio
    async def test_mixed_evaluation_types_concurrent(self, bedrock_config, sample_candidate):
        ...
        with patch('src.llm_judge.infrastructure.clients.bedrock_client.BOTO3_AVAILABLE', True):
            with patch('boto3.Session') as mock_session:
                ...
                def mock_invoke_side_effect(*args, **kwargs):
                    ...
                    if 'multi-criteria' in str(body).lower() or 'evaluations' in str(body).lower():
                        # Multi-criteria response
                        response_data = {
                            "evaluations": [
                                {"criterion": "accuracy", "score": 4, "reasoning": "Good", "confidence": 0.9}
                            ],
                            "overall_score": 4.0,
                ...
                tasks = [
                    judge.evaluate_response(sample_candidate, criteria="quality", use_multi_criteria=False),
                    judge.evaluate_response(sample_candidate, use_multi_criteria=True),
                    judge.evaluate_response(sample_candidate, criteria="accuracy", use_multi_criteria=False),
                ]
