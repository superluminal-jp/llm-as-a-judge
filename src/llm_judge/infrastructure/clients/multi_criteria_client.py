...
class MultiCriteriaEvaluationMixin:
    ...
    def _build_multi_criteria_prompt(
        self, prompt: str, response_text: str, criteria: EvaluationCriteria
    ) -> str:
...
"""
            ...
            if criterion.examples:
                ...
                if isinstance(criterion.examples, dict):
                    for score, example in criterion.examples.items():
                        desc += f"  {score}: {example}\n"
                elif isinstance(criterion.examples, list):
                    for i, example in enumerate(criterion.examples, 1):
                        desc += f"  {i}: {example}\n"
                else:
                    logger.warning(
                        f"Unexpected examples format for criterion {criterion.name}: {type(criterion.examples)}"
                    )
...
Respond with valid JSON only:"""
    ...
    def _parse_multi_criteria_response(
        self, response_text: str, criteria: EvaluationCriteria, judge_model: str
    ) -> MultiCriteriaResult:
        ...
        try:
            ...
            for score_data in response_data.get("criterion_scores", []):
                try:
                    ...
                    if not isinstance(score_data, dict):
                        logger.warning(f"Skipping non-dict score data: {score_data}")
                        continue

                    # Get the criterion definition to get weight and scale
                    criterion_name = score_data.get("criterion_name", "")
                    criterion_def = criteria.get_criterion(criterion_name)
                    ...
                    if not criterion_def:
                        ...
                        criterion_def = CriterionDefinition(
                            name=criterion_name or "unknown",
                    ...
                    try:
                        score = int(round(float(score_data.get("score", 3))))
                    except (ValueError, TypeError):
                        logger.warning(
                            f"Invalid score for {criterion_name}, using default: {score_data.get('score')}"
                        )
                        score = 3
                    ...
                    criterion_score = CriterionScore(
                        ...
                        score=score,
                        reasoning=score_data.get(
                            "reasoning", f"Evaluation for {criterion_name}"
                        ),
            ...
            if not criterion_scores:
                logger.warning(
                    "No valid criterion scores found, creating fallback scores"
                )
            ...
            try:
                ...
                if result:
                    ...
                    if isinstance(result, list):
                        logger.warning(
                            f"LLM returned a list instead of dict, wrapping in dict: {result}"
                        )
                        result = {"criterion_scores": result}
                    elif not isinstance(result, dict):
                        logger.warning(
                            f"LLM returned unexpected type {type(result)}, attempting to convert"
                        )
                        continue
    ...
    def _validate_response_structure(
        self, response_data: dict, criteria: EvaluationCriteria
    ):
        """Validate that the response has the required structure."""
        # Ensure response_data is a dictionary
        if not isinstance(response_data, dict):
            raise ValueError(
                f"Response data must be a dictionary, got {type(response_data)}"
            )
        ...
        for score in criterion_scores:
            if isinstance(score, dict):
                provided_criteria.add(score.get("criterion_name", ""))
            else:
        ...
        for i, score_data in enumerate(criterion_scores):
            if isinstance(score_data, dict):
                ...
                logger.warning(
                    f"Skipping validation for non-dict score at index {i}: {score_data}"
                )
    ...
    def _validate_criterion_score(self, score_data: dict, index: int):
        ...
        for field in required_score_fields:
            if field not in score_data:
                raise ValueError(
                    f"Criterion score {index} missing required field: {field}"
                )
        ...
        try:
            ...
            if not (1 <= score <= 5):  # Assuming 1-5 scale
                logger.warning(
                    f"Score {score} outside expected range for criterion {score_data['criterion_name']}"
                )
        except (ValueError, TypeError):
            raise ValueError(
                f"Invalid score value for criterion {score_data['criterion_name']}: {score_data['score']}"
            )
        ...
        if "confidence" in score_data:
            try:
                ...
                if not (0 <= confidence <= 1):
                    logger.warning(
                        f"Confidence {confidence} outside 0-1 range for criterion {score_data['criterion_name']}"
                    )
            except (ValueError, TypeError):
                logger.warning(
                    f"Invalid confidence value for criterion {score_data['criterion_name']}"
                )
        ...
        if not reasoning:
            logger.warning(
                f"Empty reasoning for criterion {score_data['criterion_name']}"
            )
    ...
    async def evaluate_multi_criteria(
        ...
        try:
            ...
            result = self._parse_multi_criteria_response(
                response.content, criteria, model
            )
            ...
            logger.info(
                f"Multi-criteria evaluation completed in {processing_duration:.2f}s"
            )
            return result
    ...
    async def evaluate_multi_criteria(
        ...
        try:
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are an expert evaluator conducting comprehensive multi-criteria assessments.",
                },
...
class MockMultiCriteriaClient(MultiCriteriaEvaluationMixin):
    ...
    async def evaluate_multi_criteria(
        ...
        for criterion in criteria.criteria:
            ...
            score = int(
                round(
                    max(
                        criterion.scale_min,
                        min(criterion.scale_max, base_score + variation),
                    )
                )
            )
        ...
        if avg_score >= 4:
            ...
            suggestions = [
                "Review factual content",
                "Provide more comprehensive explanation",
        ...
        if avg_score >= 4:
            ...
            overall_reasoning += (
                "This is a satisfactory response with some areas for improvement."
            )
        else:
            overall_reasoning += (
                "This response needs significant improvement across multiple criteria."
            )
...
class MultiCriteriaBedrockClient(MultiCriteriaEvaluationMixin, BedrockClient):
    ...
    async def evaluate_multi_criteria(
        ...
        evaluation_prompt = self._build_multi_criteria_prompt(
            prompt, response_text, criteria
        )
        ...
        try:
            ...
            response = await self.generate(
                messages, model=model, temperature=0.1, max_tokens=2000
            )
            ...
            return await mock_client.evaluate_multi_criteria(
                prompt, response_text, criteria, model
            )
